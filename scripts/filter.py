...
def prepare(source_file, target_file, source_lang, target_lang, lower=False):
    ...
    df_source = pd.DataFrame([i.strip() for i in open(source_file).readlines()], columns=["Source"])
    df_target = pd.DataFrame([i.strip() for i in open(target_file).readlines()], columns=["Target"])
    ...
    source_file = source_file+'.fil.txt'
    target_file = target_file+'.fil.txt'
...
prepare(source_file, target_file, None, None, lower=False)
